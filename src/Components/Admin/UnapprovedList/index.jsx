...
const headCells = [
    ...
    { id: 'duyetKhoaHoc', numeric: false, disablePadding: false, label: '' },
];
...
const UnapprovedList = (props) => {
    ...
    useEffect(() => {
        (async () => {
            try {
                //console.log(user._id)
                const rs = await courseApi.getCourseUnapproved();
                console.log(rs)
                ...
                apiCourse
                    ...
                    .then(() => {
                        Swal.fire({
                            ...
                            .then(() => { props.history.push('/admin/unapprovedlist') })
                    })
            }
    ...
    const approvedCourse = (_id) => {
        Swal.fire({
            title: 'Bạn có muốn duyệt khóa học này?',
            icon: 'warning',
            confirmButtonColor: '#e74c3c',
            confirmButtonText: 'Ok',
            showCancelButton: true
            ...
            if (result.value) {
                apiCourse
                    .put(`DuyetKhoaHoc?_id=${_id}`)
                    .then(() => {
                        Swal.fire({
                            title: 'Đã duyệt khoá học thành công!',
                            icon: 'success',
                            confirmButtonColor: '#e74c3c',
                            confirmButtonText: 'Ok',
                        })
                                ...
                                .map((row, index) => {
                                    return (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell align="left"><Button size="small" variant="contained" color="default" startIcon={<SaveIcon />} onClick={() => { approvedCourse(row._id) }}>Duyệt</Button></TableCell>
                                        </TableRow>
                                    );
                                })}
                            {emptyRows > 0 && (
...
export default connect(mapStateToProps, mapDispatchToProps)(UnapprovedList);